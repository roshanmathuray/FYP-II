        ...
        try:
            ...
            if p.exists():
                if name.lower().endswith(".csv"):
                    ...
                    return pd.read_excel(p), name
        except Exception:
            pass
    return None, None

df, used_name = load_local_dataset()

if df is None:
    st.error("Dataset not found. Place 'online_shoppers_intention.xlsx' (or CSV) in the same folder as app.py and redeploy.")
    st.stop()
...
# -----------------------------
# Basic settings (no upload needed)
# -----------------------------
target_col = "Revenue" if "Revenue" in df.columns else st.text_input("Target column name", value="Revenue")
test_size = 0.2
random_state = 42
scale_choice = "StandardScaler"  # choose from: None, StandardScaler, MinMaxScaler
n_clusters = 4
